...
**In-head priority (plumbing track, after densonly/shift diags):** (1) **low-rank δa** — PCA on train `(a_true−a_clim)`, predict ~16 scores, decode to δa, then softplus+cumsum (restore vertical coordination argo16 gets for free; monotonicity preserved); (2) **month-resolved / harmonic a_clim** (JJA clim error peaks); (3) loss already in σ₀ space — do not move it to a-space. SSH→density ablation last. Keep **v10 spice frozen**.
