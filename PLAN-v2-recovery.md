...
| Date | Change | Why |
|------|--------|-----|
| 2026-07-17 | **Physical-space CRPS ruler + C×det finding.** Prereg amended: §3 ranking uses ensemble CRPS after decode to T/S (latent CRPS diagnostic only). `scripts/phase5_physical_space_score.py` + joint_eof `eval_run` regression. **Finding:** C×det T 0.609±0.012 MISS — Phase 3 single-run 0.562 was matrix admission, not the final number (`finding_C_det_gate_overfit.md`). | Close basis-dependence hole; log over-fit gate catch. |
| 2026-07-17 | **Phase 5 matrix closed (v2):** all 9 cells scored. Survivors: A×CRPS/NLL/det, B×CRPS/NLL/det, C×NLL. Misses: C×CRPS, C×det. B×det T **0.534**. Cross-rep CRPS not comparable → `reports/ablation_summary.md` records within-rep picks + three labeled anchors (no silent single winner). `eval_run.py` joint_eof decode fixed. | Matrix GPU path done; human names default if one backbone required. |
| 2026-07-17 | **C row closed under v2:** C×NLL ENCE **0.120** (sole survivor); C×CRPS 0.248 MISS; C×det T 0.609 MISS vs floor 0.590. Fixed det launcher (`freeze_sigma` strip). Wired `PCAHeteroLoss` + twostage for A/B; A×CRPS launched. | Continue matrix; winner still provisional until A/B scored. |
| 2026-07-17 | **C×NLL v2 scored — first ENCE survivor:** cell-mean test ENCE **0.120±0.021** / CRPS 0.774±0.036 / Spearman 0.556 (clears 0.20). Best val recipe often `none`/`global`. C×det v2 launched. | Matrix continues; §3.1 may be unnecessary if NLL remains sole/best survivor. |
| 2026-07-17 | **C×CRPS v2 scored:** cell-mean test ENCE **0.248±0.021** / CRPS 0.742±0.048 / Spearman 0.632 — does **not** clear 0.20; does **not** recover Phase 4 s2b (0.160). Val post-α ENCE ~0.02–0.03. Stopping on train-time raw val ENCE ≠ s2b fixed-budget maturity. Recorded; no re-tune. C×NLL v2 launched under same locked protocol. | Proceed matrix; §3.1 fallback still armed. |
| 2026-07-17 | **Phase 5 protocol v2 amendment (ACK'd):** stage-2 early-stops on **val ENCE** (patience 40; ceiling stage-1+190), never on val loss/CRPS, uniformly for all prob cells. Prereg mistranscribed Phase 4's s2b-class calibration-mature schedule as generic loss early-stop; matrix v1 C×CRPS mean ENCE 0.225 replicated Phase 4 short-s2 failure (0.231), independent of which cell won. C×NLL v1 aborted mid-stage-2; v1 C×CRPS retained as labeled evidence. No-survivors fallback pre-registered before v2 scores. Threshold stays 0.20; cell judgment = mean±std not per-seed counts. | Human ACK: restore fidelity to pre-existing Phase 4 finding, not tune toward test. |
