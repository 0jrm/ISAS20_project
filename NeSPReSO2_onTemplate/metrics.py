...
def error_blend_from_series(e_nes: np.ndarray, e_xb: np.ndarray) -> dict[str, float]:
    """rho, w*, and RMSEs from two 1-d error series (same length, finite pairs)."""
    ...
    return {
        ...
        "rmse_xb": rmse_xb,
        "rmse_nes": rmse_nes,
        "rmse_blend": blended_rmse(rmse_nes, rmse_xb, rho, w),
    }
...
def bootstrap_blend_over_casts(
    ...
    *,
    ...
    """A1 cell stats. Bootstrap resamples casts. ``ci_lo``/``ci_hi`` are on blend RMSE."""
    e_nes = np.asarray(e_nes, dtype=np.float64)
    e_xb = np.asarray(e_xb, dtype=np.float64)
    valid = np.asarray(valid, dtype=bool)
    n_cast = e_nes.shape[0]
    if pool == "levels":
        en0, ex0 = pooled_band_errors(e_nes, e_xb, valid, z, z_lo, z_hi)
    elif pool == "cast_mean":
        en0, ex0 = cast_mean_band_errors(e_nes, e_xb, valid, z, z_lo, z_hi)
    else:
    ...
    for _ in range(int(n_boot)):
        idx = rng.integers(0, n_cast, size=n_cast)
        if pool == "levels":
            en, ex = pooled_band_errors(e_nes[idx], e_xb[idx], valid[idx], z, z_lo, z_hi)
        else:
            en, ex = cast_mean_band_errors(e_nes[idx], e_xb[idx], valid[idx], z, z_lo, z_hi)
        boots.append(error_blend_from_series(en, ex)["rmse_blend"])
    boots = np.asarray(boots, dtype=np.float64)
    finite = boots[np.isfinite(boots)]
    if finite.size:
        point["ci_lo"] = float(np.percentile(finite, 2.5))
        point["ci_hi"] = float(np.percentile(finite, 97.5))
    else:
        point["ci_lo"] = float("nan")
        point["ci_hi"] = float("nan")
    point["n_cast"] = float(n_cast)
    return point
