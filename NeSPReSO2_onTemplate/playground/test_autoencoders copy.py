...
def plot_residuals_grid(original, ae_recons, pca_recons, mask, encoding_dims, depth_list, parameter_name, depth_limit=2000):
...
def main():
    ...
    for parameter_name in parameter_names:
        ...
        # ------------------------------------------------------------------
        # Recover the mean ("average") profile that was removed during
        # preprocessing so that we can display results on the original
        # physical scale.
        # ------------------------------------------------------------------
        valid_vals = (~mask).float()
        depth_counts = torch.sum(valid_vals, dim=0)
        depth_counts = torch.clamp(depth_counts, min=1)  # avoid divide-by-zero
        mean_profile = torch.sum(param_data * valid_vals, dim=0) / depth_counts
        mean_profile[torch.isnan(mean_profile)] = 0.0
        mean_profile = mean_profile.unsqueeze(0)  # (1, D) for broadcasting
        ...
        # Un-normalised copies for plotting/statistics
        vis_data_unnorm = vis_data + mean_profile
        # For residuals, use all test profiles
        test_data = torch.stack([test_dataset[i][0] for i in range(len(test_dataset))])
        test_mask = torch.stack([test_dataset[i][1] for i in range(len(test_dataset))])
        # Un-normalised copies for plotting/statistics
        test_data_unnorm = test_data + mean_profile
        ...
        for layout, layout_name in zip(layer_layouts, layout_names):
            ...
            for dim in encoding_dims:
                ...
                masked_ae.eval()
                with torch.no_grad():
                    vis_data_device = vis_data.to(device)  # keep normalised input for AE
                    vis_mask_device = vis_mask.to(device)
                    masked_recon = masked_ae(vis_data_device, vis_mask_device).cpu()
                    # Bring back to original physical scale for plotting
                    masked_recon_unnorm = masked_recon + mean_profile
                    # For residuals, reconstruct all test profiles
                    test_data_device = test_data.to(device)  # keep normalised input for AE
                    test_mask_device = test_mask.to(device)
                    masked_recon_test = masked_ae(test_data_device, test_mask_device).cpu()
                    masked_recon_test_unnorm = masked_recon_test + mean_profile
                all_reconstructions.append(masked_recon_unnorm)
                ...
                # Un-normalise PCA outputs
                pca_recon_unnorm = pca_recon + mean_profile
                pca_recon_test_unnorm = pca_recon_test + mean_profile
                all_pca_reconstructions.append(pca_recon_unnorm)
                # Variance (optional, not used in plots)
                captured_var = calculate_captured_variance(vis_data_unnorm, masked_recon_unnorm, vis_mask)
                pca_var = calculate_captured_variance(vis_data_unnorm, pca_recon_unnorm, vis_mask)
                ...
                if dim == encoding_dims[0]:
                    all_reconstructions_test = [masked_recon_test_unnorm]
                    all_pca_reconstructions_test = [pca_recon_test_unnorm]
                else:
                    all_reconstructions_test.append(masked_recon_test_unnorm)
                    all_pca_reconstructions_test.append(pca_recon_test_unnorm)
            ...
            plot_overlay_profiles_ae_pca(vis_data_unnorm, all_reconstructions, all_pca_reconstructions, vis_mask, f"{parameter_name} (layout {layout_name})", encoding_dims, depth_list, num_vis_profiles)
            plot_residuals_grid(test_data_unnorm, all_reconstructions_test, all_pca_reconstructions_test, test_mask, encoding_dims, depth_list, parameter_name)

            # Best/Worst plots per encoding dimension
            for idx_dim, dim in enumerate(encoding_dims):
                plot_best_worst_profiles(test_data_unnorm, test_mask,
