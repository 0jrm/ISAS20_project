#!/bin/bash
# geoff: Stochastic EOF loss on z32+19-ops inputs (same cache as A_CRPS_z32_ops).
set -euo pipefail
...
CFG=config/argo/config_argo_geoff.json
TAG=geoff_s42
CKPT=saved/geoff/models/NeSPReSO2_ARGO_GoM_geoff_${TAG}_s2/${TAG}_s2/model_best.pth
LOG=saved/log/tmux_geoff.log
mkdir -p "$(dirname "$LOG")"
exec > >(tee -a "$LOG") 2>&1
...
"$PY" eval_run.py \
  -c "$CFG" -r "$CKPT" --split test --out ../reports/eval_geoff_s42.json

"$PY" scripts/eval_acrps_phys.py \
  -c "$CFG" -r "$CKPT" --out ../reports/eval_geoff_cal.json
