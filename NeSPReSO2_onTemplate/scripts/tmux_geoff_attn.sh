#!/bin/bash
# geoff inputs + residual input self-attention (token per scalar, skip around MHA).
set -euo pipefail
...
CFG=config/argo/config_argo_geoff_attn.json
TAG=geoff_attn_s42
CKPT=saved/geoff_attn/models/NeSPReSO2_ARGO_GoM_geoff_attn_${TAG}_s2/${TAG}_s2/model_best.pth
LOG=saved/log/tmux_geoff_attn.log
mkdir -p "$(dirname "$LOG")"
exec > >(tee -a "$LOG") 2>&1
...
if [[ -n ${RESUME_CKPT:-} ]]; then
  ...
  "$PY" selfcheck.py test_input_self_attention_residual test_stoch_eof_recipe test_heave_ablation_pin_arch_dims
fi
...
"$PY" eval_run.py \
  -c "$CFG" -r "$CKPT" --split test --out ../reports/eval_geoff_attn_s42.json

"$PY" scripts/eval_acrps_phys.py \
  -c "$CFG" -r "$CKPT" --out ../reports/eval_geoff_attn_cal.json
