...
srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 \
  python3 train.py -c config_isas.json -id "${RUN_ID}_isas20" -d 1 \
  > "$RUN_ROOT/isas20.log" 2>&1 &
PID_ISAS=$!

srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 \
  python3 train.py -c config_argo.json -id "${RUN_ID}_argo_v2" -d 2 \
...
manifest = {
    "run_id": run_id,
    "runs": [
        {"tag": "isas20", "config": "config_isas.json", "pid": int(pid_isas)},
        {"tag": "argo_v2", "config": "config_argo.json", "pid": int(pid_argo)},
    ],
}
