...
srun --ntasks=1 --cpus-per-task=8 python3 train.py -c config/isas/config_isas.json
srun --ntasks=1 --cpus-per-task=8 python3 train.py -c config/argo/config_argo.json

# test-split eval (raw profile RMSE — use matching config + checkpoint)
srun --ntasks=1 --cpus-per-task=8 python3 eval_run.py -c config/isas/config_isas.json -r saved/models/.../checkpoint.pth
```
...
|--------|-------------------|----------|------------|
| `config/isas/config_isas.json` | `isas20` | ISAS HDF5 | 187 levels |
| `config/argo/config_argo.json` | `argo_v2` | v2 pickle | 1801 m (0–1800) |
...
CLI override: `python3 train.py -c config/isas/config_isas_patch.json --bs 0`

```bash
# Sweep powers-of-two up to max-fit; writes JSON under saved/benchmarks/
srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 \
  python3 scripts/benchmark_batch_size.py -c config/isas/config_isas_patch.json

srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 \
  python3 scripts/benchmark_batch_size.py -c config/argo/config_argo.json
```
...
|---|---:|---:|---:|---:|---:|
| `config/isas/config_isas_patch.json` (PatchConvMLP) | 3530 | 3353 | 3353 | ~335 MiB | — |
| `config/isas/config_isas.json` (PredictionModel) | 3530 | 3353 | 3353 | ~68 MiB | — |
| `config/argo/config_argo.json` (PatchConvMLP + PCA loss) | 2901 | 2755 | 2755 | ~215 MiB | ~4% (0.0233→0.0223 s/epoch) |

`config/isas/config_isas_patch.json` ships with `batch_size: 0` (auto max). `config/argo/config_argo.json` keeps `batch_size: 512` for v2 parity (max batch benchmarked but below 10% full-step bar).
Edit paths in `config/argo/config_argo.json` (`v2_pickle`, `v2_src`) for your machine.
...
{"run_id":"$RUN_ID","runs":[
  {"tag":"isas20","config":"config/isas/config_isas.json","pid":null},
  {"tag":"argo_v2","config":"config/argo/config_argo.json","pid":null}
]}
EOF
...
srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 python3 train.py -c config/isas/config_isas.json -id ${RUN_ID}_isas20 &
srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 python3 train.py -c config/argo/config_argo.json -id ${RUN_ID}_argo_v2 &
...
# when both done:
python3 eval_run.py -c config/isas/config_isas.json -r saved/models/NeSPReSO2_ISAS_GoM/${RUN_ID}_isas20/model_best.pth --out saved/runs/$RUN_ID/eval_isas.json
python3 eval_run.py -c config/argo/config_argo.json -r saved/models/NeSPReSO2_ARGO_GoM/${RUN_ID}_argo_v2/model_best.pth --out saved/runs/$RUN_ID/eval_argo.json
```
...
|---|---|---:|---:|
| PatchConvMLP 16-PC ISAS patch | `config/isas/config_isas_patch.json` | 1.016 | **5.32** |
| PredictionModel 15-PC ISAS point | `config/isas/config_isas.json` | **1.002** | 5.53 |
| PatchConvMLP 16-PC ARGO point | `config/argo/config_argo.json` | **0.416** | **0.072** |
...
srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 \
  python3 scripts/benchmark_ml_opts.py -c config/isas/config_isas.json --profile

# Or use the launcher (single-GPU + optional DDP)
bash scripts/launch_benchmark.sh config/isas/config_isas.json

# DDP only (2 GPUs)
srun --ntasks=1 --cpus-per-task=8 --gres=gpu:2 \
  torchrun --nproc_per_node=2 scripts/benchmark_ml_opts.py \
  -c config/isas/config_isas.json --variant ddp
```
...
**ISAS (`config/isas/config_isas.json`)** — baseline `0.0256 s/epoch`:
...
**ARGO (`config/argo/config_argo.json`)** — baseline `0.0214 s/epoch` (deeper PCA loss, 1801 levels):
...
ARGO configs use `split_mode: chronological`. Explicit date ranges: `config/argo/config_argo_chrono_dates.json`.

## Comparison notebook

[`notebooks/compare_v2_vs_template.ipynb`](notebooks/compare_v2_vs_template.ipynb) compares **PCA vs AE** surface encodings on a pinned random split.

### Encoding compare study (6 models)

| Label | Encoding | Surface |
|-------|----------|---------|
| ISAS-pt-PCA16 / ISAS-pch-PCA16 | PCA-16 | point MLP / patch conv |
| ISAS-pt-AE128 / ISAS-pch-AE128 | frozen AE-128 | point MLP / patch conv |
| ARGO-PCA15 / ARGO-PCA16 | PCA-15 / PCA-16 | point PatchConvMLP |

Configs live in [`config/`](config/) — see [`config/README.md`](config/README.md). Encoding-compare subset: [`config/compare/`](config/compare/). **PCA-16 and AE-128 are compared in profile RMSE space** (common 0–1800 m grid), not by latent dimension count.

**Run order:**

```bash
cd NeSPReSO2_onTemplate
# Phase A: train ISAS AE-128 + export latents, then Phase B: 6 parallel surface trains
bash scripts/run_encoding_compare_train.sh

# interactive notebook (uses saved/compare_runs/<key>/ checkpoints only)
jupyter notebook notebooks/compare_v2_vs_template.ipynb

# headless eval + overlay PNG export (when compare checkpoints exist)
...
All statistics are defined in [`notebooks/nb_metrics.py`](notebooks/nb_metrics.py) (`STATISTICS` contract). Summary tables use `raw_profile_rmse_common`; native-grid RMSE matches `eval_run.py`. Decoder models decode via frozen AEs before scoring.
