...
**CLOSE-OUT COMPLETE (2026-07-15):** [`PLAN-agentic-close-out.md`](PLAN-agentic-close-out.md) — **all 6 steps done**; session record: [`HANDOFF-2026-07-15-agentic-close-out.md`](HANDOFF-2026-07-15-agentic-close-out.md). Two findings outrank the plan's own questions:
1. **🚨 "nature's 24.70%" was never nature** — it is the **PCA-16 regression target**. Nature = **1.12%** (test) / 3.88% (all). *The basis truncation, not the model, is the dominant source of σ₀ inversions* (1.12% → 21.83%, 6.4×). Track 0's "not a PCA artifact" control compared the number **to itself**. ⇒ **`PLAN.md` Phase 8 CLOSED.**
2. **🚨 The anomaly parity gap is ~62% smaller than reported** — eval-mode inference under-predicts **anomaly** PC1 by ~16–18% (MC averaging recovers it; raw models unaffected). Like-for-like: `retune_anom` **0.5900** vs `golden_point` **0.5360** (gap 0.1436 → 0.0540). Loss scales were **not** the cause (closed-negative, 18%).

Also: **RC-4 is live** (models over-confident, spread-error ratio 0.37/0.32 — but the spread is **not DA-usable**: within-depth rank corr ≈0.12); **`export_field_product.py` runs** (5 bugs, incl. 2 that made field *training* impossible) but its output is **NOT physically plausible** — the climatology extrapolates outside the ARGO hull (43.8 PSU at ocean points). **Open:** cube SSS degradation; `point_cube` best_epoch=27; clip field grid to ARGO hull; `dropout_prob` for anomaly configs.
...
**Loss-scale retrain: DONE, closed-negative (2026-07-15 16:04).** `retune_0715_anom_point`, early stop epoch 3382. T RMSE **0.6803 → 0.6545** (−3.79%) — only **18%** of the parity gap, exactly as pre-registered. Keep the corrected scales (analytically right); **stop blaming `loss_scales`**. ⚠️ `val_loss` is **not** comparable across the two runs (the correction changed the loss normalization); use `raw_profile_rmse_native` only.
**🚫 `PLAN.md` Phase 8 is CLOSED — do not run it (diagnosed 2026-07-15, [`HANDOFF-2026-07-15-agentic-close-out.md`](HANDOFF-2026-07-15-agentic-close-out.md) Step 4).** ⚠️ **"nature's 24.7%" was never nature** — it is the **PCA-16 regression target**; raw ARGO violates at **1.12%** (test) / 3.88% (n=4145), and the PCA-16 target at **21.83%**, so *the basis truncation, not the model, is the dominant source of σ₀ inversions* (6.4×). `point_cube`'s 38.5% is a **feature-pipeline defect**: 100% of its violations sit at 2.5–11.5 m, all marginal (0.010–0.02 kg/m³, gone by tol=0.03), because it reproduces the near-surface halocline 10× too weakly (dS(0→15 m) +0.016 vs +0.14) — the cube's **SSS** correlates only 0.744 with the point cache's (SST: 0.9885). `residual_cube` escapes by anchoring on the point block's original SSS. A σ₀ penalty would make it fabricate a halocline it cannot place. **Fix the cube SSS; investigate `point_cube` best_epoch=27.**
...
| 8 Physics loss | 14 | **🚫 CLOSED — do not run (2026-07-15)** | Diagnosed. "Nature's 24.70%" was the **PCA-16 target**; nature is **1.12%** (test). The **basis**, not the model, injects the inversions (1.12% → 21.83%) — a loss on model output can't fix an unstable target. All violations are marginal (0.010–0.02 kg/m³, gone by tol=0.03). `point_cube`'s 38.52% is a **cube-SSS feature defect** (missing near-surface halocline), not physics. |
